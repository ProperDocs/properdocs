...
class FilesystemObjectTest(TestCase):
    ...
    def test_docs_dir_is_config_dir_fails(self) -> None:
        ...
        with self.expect_error(
            docs_dir="""The 'properdocs.yml' config file should not be inside the docs_dir.
...
exclude_docs: |
  /properdocs.yml"""
    ...
    def test_docs_dir_is_config_dir_nested_fails(self) -> None:
        ...
        with self.expect_error(
            docs_dir="""The 'properdocos.yaml' config file should not be inside the docs_dir.
To allow this arrangement, please exclude the file (and other files as applicable) by adding the following configuration:

exclude_docs: |
  /config/properdocos.yaml"""
        ):
            self.get_config(
                Schema,
                {'docs_dir': '..'},
                config_file_path=os.path.join(
                    os.path.abspath('.'), 'docs', 'config', 'properdocos.yaml'
                ),
            )
    ...
    def test_docs_dir_is_config_dir_succeeds(self) -> None:
        ...
        conf = self.get_config(
            Schema,
            {'docs_dir': '.', 'exclude_docs': '/properdocs.y*l'},
            config_file_path=os.path.join(os.path.abspath('.'), 'properdocs.yml'),
        )
